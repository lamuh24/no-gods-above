...
export const inputActions = ["left", "right", "down", "up", "light", "medium", "heavy", "special", "ultimate", "romanCancel", "throw", "block", "burst", "pause", "reset"] as const;
...
export function wasPressedWithHeld(buffer: DeterministicInputBuffer, action: InputAction, required: InputAction, leniency = 1) { return buffer.history.slice(-leniency).some((h) => !!h.pressed[action] && !!h.held[required]); }
