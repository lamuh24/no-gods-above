...
Owner manual playtest of `2H -> jump cancel -> j.J -> j.K -> j.L`, including direct `j.J -> j.L` and `j.K -> j.L`. If accepted, the next task is universal forward-throw architecture, followed by temporary rigged Lamuh integration. Do not advance to either within this task.
