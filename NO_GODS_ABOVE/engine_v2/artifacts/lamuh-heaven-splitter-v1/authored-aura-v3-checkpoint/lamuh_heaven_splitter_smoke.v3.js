...
// Heavy V3 sprites include the authored aura, so their bounds are COMPOSITE bounds.
...
(async () => {
  ...
  const report = { createdAt: new Date().toISOString(), version: 'authored-aura-v3', candidateOnly: true, deployable: false, strengths: selected, cases: [], keyboard: [], cornerContainment: [], failures: [], browserErrors: [], ignoredHotReloads: [], renderedLayerBoundsMethod: 'Actual prepared sprite alpha plus CanvasRenderingContext2D current transform, sampled every simulation tick. Heavy V3 includes the authored aura: these are composite bounds, not isolated body/anatomy bounds. Opaque body preservation receives a separate exact-pixel audit.', recordings: [] };
  try {
    ...
    await openSandbox();
    const heavy = await page.evaluate(async () => (await (await fetch('/lamuh-legacy-v2/review-data.json', { cache: 'no-store' })).json()).heavenSplitterFamily.variants.heavy);
    const bakedFrames = heavy.v2.frames.filter((frame) => frame.auraBaked);
    assert(bakedFrames.length >= 5, 'Heavy must use authored aura source frames');
    for (const side of ['p1', 'p2']) for (const e of family) for (const outcome of ['hit', 'stand_block', 'crouch_block', 'whiff', 'anti_air']) {
      ...
      try {
        ...
        for (let i = 0; i < e.total + 32; i++) {
          ...
          const extraPhase = e.strength === 'heavy' && actor.currentAttack ? snapshot.readout.activeNormalFrame === 3 ? 'aura-coil' : snapshot.readout.activeNormalFrame === 4 ? 'aura-rising-connector' : snapshot.readout.activeNormalFrame === 8 ? 'post-apex-gather' : null : null;
    ...
    report.authoredAuraPixelAudit = await page.evaluate(async (records) => {
      const results = [];
      ...
      for (const record of records) {
        ...
        let opaquePixels = 0, changedOpaquePixels = 0, auraOutsideBodyPixels = 0;
        for (let i = 0; i < body.data.length; i += 4) {
          if (body.data[i + 3] === 255) { opaquePixels++; if (composite.data[i] !== body.data[i] || composite.data[i + 1] !== body.data[i + 1] || composite.data[i + 2] !== body.data[i + 2] || composite.data[i + 3] !== 255) changedOpaquePixels++; }
          if (body.data[i + 3] === 0 && composite.data[i + 3] > 0) auraOutsideBodyPixels++;
        }
        results.push({ index: record.index, actualCompositeSha256: await hash(record.publicPath), expectedCompositeSha256: record.sha256, actualBodySha256: await hash(record.bodyOnlyPublicPath), expectedBodySha256: record.bodyOnlySha256, opaquePixels, changedOpaquePixels, auraOutsideBodyPixels });
        composite.canvas.width = body.canvas.width = 1; composite.image.src = body.image.src = '';
      }
      return results;
    }, bakedFrames);
    for (const row of report.authoredAuraPixelAudit) {
      assert.strictEqual(row.changedOpaquePixels, 0, `Aura frame${row.index} changed opaque body pixels`);
      assert.strictEqual(row.actualCompositeSha256, row.expectedCompositeSha256);
      assert.strictEqual(row.actualBodySha256, row.expectedBodySha256);
    }
    assert.strictEqual(report.authoredAuraPixelAudit.filter((row) => row.auraOutsideBodyPixels > 0).length, 5, 'Five authored aura poses must contain visible source pixels outside the preserved body');
    ...
    await page.locator('#stage').screenshot({ path: path.join(out, 'authored-aura-v3-on.png') });
    ...
    assert.strictEqual(off.draws[0].sourcePath, contactRecord.bodyOnlyPublicPath, 'VFX-off must select the preserved body-only PNG');
    ...
    await page.locator('#stage').screenshot({ path: path.join(out, 'authored-aura-v3-off.png') });
    await page.locator('#vfx').check();
    report.authoredAuraSelection = { actualCompositePath: auraSnapshot.bodies[0].sourcePath, bodyFallbackPath: off.draws[0].sourcePath, noProceduralOverlay: true, togglePreservedSimulation: true, screenshots: ['authored-aura-v3-on.png', 'authored-aura-v3-off.png'] };
    console.log('PASS authored source path, twelve exact pixel/hash pairs, five aura poses, no double overlay and VFX-off fallback');
    ...
    await page.locator('#v2').screenshot({ path: path.join(out, 'comparison-heavy-authored-aura-v3.png') });
    ...
    report.comparison.authoredAura = { actualCompositePath: reviewOn.sourcePath, actualBodyOnlyPath: reviewOff.sourcePath, noDoubleOverlay: true };
