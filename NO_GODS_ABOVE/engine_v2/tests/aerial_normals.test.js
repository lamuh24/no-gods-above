...
function jump(state, directional = {}) {
  ...
  assert.strictEqual(state.fighters.p1.airActionsRemaining, defaultTuning.combat.airActionBudget);
}
function isolatedAirPair(seed = 700) {
  const state = createMatch(seed); const p1 = state.fighters.p1; const p2 = state.fighters.p2;
  Object.assign(p1, { x: -40, y: -80, vx: 1.5, vy: -2, grounded: false, phase: 'jump', phaseTick: 0, airActionsRemaining: defaultTuning.combat.airActionBudget });
...
function testLauncherToAirLightMediumHeavyConnects() {
  ...
  assert.strictEqual(p1.comboCount, 4); assert.strictEqual(p1.comboDamage, 194); assert.strictEqual(p1.airActionsRemaining, 2);
...
function testAirMediumToHeavyConnects() {
  ...
  assert.deepStrictEqual(state.fighters.p1.cancelOptions, ['air_light', 'air_heavy']);
  tick(state, { p1: { heavy: true } }); waitForHit(state, 2);
  assert.deepStrictEqual(state.fighters.p1.comboRoute, ['air_medium', 'air_heavy']);
}

function testHeavyReverseAerialChainsAreRejected() {
  for (const [first, attempted, expected] of [[{ heavy: true }, { light: true }, 'air_heavy'], [{ heavy: true }, { medium: true }, 'air_heavy']]) {
...
function testAirActionBudgetPreventsAerialLoops() {
  const completed = launcherAerialRoute(716); assert.strictEqual(completed.fighters.p1.airActionsRemaining, 2);
...
const tests = [
  ...
  testHeavyReverseAerialChainsAreRejected,
