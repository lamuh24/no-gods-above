#!/usr/bin/env python3
"""Package a generated Seris Sheet 1 draft into the runtime-compatible atlas.

This is an asset-prep helper only. It reads a generated 8x6 chroma-key preview,
removes the key/grid, and writes the exact 3072x2304 transparent Sheet 1 atlas.
It does not change gameplay, mappings, or runtime asset paths.
"""
...
def package(input_path: Path, output_path: Path, report_path: Path) -> dict:
    ...
    if abs(src_cell_w - round(src_cell_w)) > 0.01 or abs(src_cell_h - round(src_cell_h)) > 0.01:
        warnings.append(f"Source does not divide cleanly into 8x6: cell approx {src_cell_w:.3f}x{src_cell_h:.3f}")
    ...
    for row in range(ROWS):
        for col in range(COLS):
            ...
            # Trim 2px dividers from generated preview cells when present.
            trim = 2
            ...
            paste_x = max(col * CELL + 16, min(paste_x, (col + 1) * CELL - scaled_w - 16))
...
def main() -> int:
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=Path("assets/sprites/seris_revamp/seris_sheet_1_core_movement_revamp_atlas.png"),
    )
    parser.add_argument(
        "--report",
        type=Path,
        default=Path("assets/sprites/seris_revamp/seris_sheet_1_core_movement_revamp_pack_report.json"),
    )
