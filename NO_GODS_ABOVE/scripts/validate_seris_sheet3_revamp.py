#!/usr/bin/env python3
"""Validate a clean Seris Sheet 3 revamp atlas before runtime wiring."""
...
def validate(input_path: Path, out_dir: Path, root: Path) -> int:
    ...
    report = {
        ...
        "contract": {
            "sheet": "Seris Sheet 3 - Ground Normals",
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Validate Seris Sheet 3 revamp atlas.")
    parser.add_argument("input", type=Path, help="Path to generated Seris Sheet 3 PNG.")
    parser.add_argument(
        "--out-dir",
        type=Path,
