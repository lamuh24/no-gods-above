#!/usr/bin/env python3
"""Validate a clean Seris Sheet 5 revamp atlas before runtime wiring."""
...
def analyze_cells(arr: np.ndarray) -> tuple[list[dict[str, Any]], list[str], list[str]]:
    ...
    for row in range(ROWS):
        for col in range(COLS):
            ...
            white = alpha & (cell_i[:, :, 0] > 230) & (cell_i[:, :, 1] > 230) & (cell_i[:, :, 2] > 230)
            bright_low_sat = alpha & (cell_i[:, :, 0] > 150) & (cell_i[:, :, 1] > 150) & (cell_i[:, :, 2] > 150) & ((mx - mn) < 80)
            ...
            grid_like.extend(vertical_grid)
...
def validate(input_path: Path, out_dir: Path, root: Path) -> int:
    ...
    report = {
        ...
        "contract": {
            "sheet": "Seris Sheet 5 - Specials Body",
            "atlas": [ATLAS_W, ATLAS_H],
            "cell": [CELL_W, CELL_H],
            "grid": [COLS, ROWS],
            "frameCounts": FRAME_COUNTS,
            "runtime": {"fixedSourceCells": True, "skipSanitize": True, "anchorX": 320, "baselineY": 406},
        },
        "checks": {
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Validate Seris Sheet 5 revamp atlas.")
