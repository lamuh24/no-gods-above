#!/usr/bin/env python3
"""Package a generated Seris Sheet 4 draft into the runtime-compatible atlas.

This is an asset-prep helper only. It removes the chroma/grid preview and writes
the exact 5824x1792 transparent Sheet 4 atlas. It does not change runtime code.
"""
...
ROW_NAMES = ["air_light", "air_medium", "air_heavy", "air_recovery"]
SIX_COLUMN_SOURCE_MAP = {
    0: [0, 1, 2, 3],
    1: [0, 1, 2, 3, 4, 5],
    2: [0, 1, 2, 3, 4, 4, 5],
    3: [0, 1, 2, 2],
...
def remove_preview_grid_fragments(atlas: Image.Image) -> tuple[Image.Image, int]:
    ...
    g = arr_i[:, :, 1]
    b = arr_i[:, :, 2]
    bright_low_sat = alpha & (r > 150) & (g > 150) & (b > 150) & ((np.maximum.reduce([r, g, b]) - np.minimum.reduce([r, g, b])) < 80)
...
def package(input_path: Path, output_path: Path, report_path: Path) -> dict:
    ...
    use_detected_grid = detected_source_cols in (COLS, 6)
    if not use_detected_grid and (abs(src_cell_w - round(src_cell_w)) > 0.01 or abs(src_cell_h - round(src_cell_h)) > 0.01):
        warnings.append(f"Source does not divide cleanly into 7x4: cell approx {src_cell_w:.3f}x{src_cell_h:.3f}")
    if use_detected_grid:
        warnings.append("Source preview used detected grid-line boundaries instead of equal source-cell slicing.")
    if use_detected_grid and detected_source_cols == 6:
        warnings.append("Six-column source preview expanded into the 7-column runtime contract by holding one air_heavy follow-through/recoil frame.")
    ...
    for row in range(ROWS):
        for col in range(COLS):
            ...
            source_col = col
            if use_detected_grid and detected_source_cols == 6:
                source_col = SIX_COLUMN_SOURCE_MAP[row][col]
            ...
            if use_detected_grid:
                ...
                x0 = int(round(col * src_cell_w)) + 6
                y0 = int(round(row * src_cell_h)) + 6
                x1 = int(round((col + 1) * src_cell_w)) - 6
                y1 = int(round((row + 1) * src_cell_h)) - 6
            raw_cell = source.crop((x0, y0, max(x0 + 1, x1), max(y0 + 1, y1)))
            cleaned = clean_cell(raw_cell)
            if use_detected_grid and detected_source_cols == 6 and row == 2 and source_col == 4:
                cleaned_arr = np.asarray(cleaned).copy()
                cleaned_arr[:, :24, 3] = 0
...
def main() -> int:
    ...
    parser.add_argument(
        "--report",
        type=Path,
        default=Path("assets/sprites/seris_revamp/seris_sheet_4_air_normals_revamp_pack_report.json"),
    )
