#!/usr/bin/env python3
"""Normalize LAMUH Super / Ascended golden-locs source into a 448-cell runtime atlas."""
...
ROW_MAPPING = {
    "0": ["lamuh_super_idle", "crown_recovery"],
    "1": ["lamuh_super_activation", "crown_startup", "crown_charge", "ultimate"],
    "2": ["crown_rush"],
    "3": ["crown_combo_a"],
    "4": ["crown_combo_b", "crown_launch"],
    "5": ["lamuh_super_attack_overlay", "crown_fire"],
}
...
def build_clustered_slot_crops(src: Image.Image) -> tuple[dict[tuple[int, int], Image.Image], dict[str, object]]:
    ...
    for row, row_cluster in enumerate(sorted(row_clusters, key=lambda cluster: median([float(comp["center"][1]) for comp in cluster]))):
        ...
        sorted_col_clusters = sorted(col_clusters, key=lambda group: median([float(comp["center"][0]) for comp in group]))
        if len(sorted_col_clusters) > COLS:
            raise RuntimeError(f"Expected at most {COLS} body slots in row {row}, detected {len(sorted_col_clusters)}")
        expected_centers = [((col + 0.5) * cleaned.width / COLS) for col in range(COLS)]
        assigned_clusters: dict[int, list[dict[str, object]]] = {}
        for cluster in sorted_col_clusters:
            cluster_center = median([float(comp["center"][0]) for comp in cluster])
            col = min(
                range(COLS),
                key=lambda candidate: (
                    abs(expected_centers[candidate] - cluster_center),
                    0 if candidate not in assigned_clusters else 1,
                ),
            )
            if col in assigned_clusters:
                assigned_clusters[col].extend(cluster)
            else:
                assigned_clusters[col] = list(cluster)
        row_center_y = float(median([float(comp["center"][1]) for comp in row_cluster]))
        for col in range(COLS):
            cluster = assigned_clusters.get(col, [])
            boxes = [comp["bbox"] for comp in cluster]
            if boxes:
                center_x = float(median([float(comp["center"][0]) for comp in cluster]))
                body_bounds = list(merge_box(boxes, 0, cleaned.width, cleaned.height))
            else:
                center_x = float(expected_centers[col])
                body_bounds = list(source_cell_bounds(cleaned.width, cleaned.height, col, row))
            repair_from = None
            if not boxes:
                repair_from = [row, max(0, col - 1)]
            slot_defs.append({
                "row": row,
                "col": col,
                "center": [
                    center_x,
                    float(median([float(comp["center"][1]) for comp in cluster])) if cluster else row_center_y,
                ],
    ...
    for row in range(ROWS):
        for col in range(COLS):
            ...
            repaired_from = None
            if (row, col) in FORCED_REPAIR_SLOTS:
                repaired_from = list(FORCED_REPAIR_SLOTS[(row, col)])
                repair_slot = slot_lookup[(int(repaired_from[0]), int(repaired_from[1]))]
                bounds = merge_box([tuple(repair_slot["body_bounds"])], 12, cleaned.width, cleaned.height)  # type: ignore[arg-type]
                body_component_count = 1
            elif not boxes or int(slot["body_component_count"]) == 0:
                repaired_from = slot.get("repair_from") or [row, max(0, col - 1)]
                repair_slot = slot_lookup[(int(repaired_from[0]), int(repaired_from[1]))]
                bounds = merge_box([tuple(repair_slot["body_bounds"])], 12, cleaned.width, cleaned.height)  # type: ignore[arg-type]
                body_component_count = 1
            else:
                body_bounds = tuple(slot["body_bounds"])  # type: ignore[arg-type]
                bounds = merge_box([body_bounds], 12, cleaned.width, cleaned.height)
...
def count_golden_locs_pixels(img: Image.Image, body: tuple[int, int, int, int] | None) -> int:
    if not body:
        return 0
    rgba = img.convert("RGBA")
    pix = rgba.load()
    x1, y1, x2, y2 = body
    upper_limit = y1 + max(1, round((y2 - y1) * 0.42))
    total = 0
    for y in range(y1, min(y2, upper_limit)):
        for x in range(x1, x2):
            pixel = pix[x, y]
            r, g, b, a = pixel
            if a > 0 and is_gold_candidate(pixel) and r >= 150 and g >= 95 and b <= 130:
                total += 1
    return total
...
def remove_far_detached_fragments(img: Image.Image) -> Image.Image:
    comps = [comp for comp in connected_components(img) if int(comp["area"]) >= 8]
    if len(comps) <= 1:
        return img
    main = max(comps, key=lambda comp: int(comp["area"]))
    main_bbox = main["bbox"]  # type: ignore[assignment]
    expanded = (
        max(0, int(main_bbox[0]) - 34),
        max(0, int(main_bbox[1]) - 34),
        min(img.width, int(main_bbox[2]) + 34),
        min(img.height, int(main_bbox[3]) + 34),
    )
    keep = []
    for comp in comps:
        bbox = comp["bbox"]  # type: ignore[assignment]
        intersects = not (
            int(bbox[2]) < expanded[0]
            or int(bbox[0]) > expanded[2]
            or int(bbox[3]) < expanded[1]
            or int(bbox[1]) > expanded[3]
        )
        if comp is main or intersects or int(comp["area"]) >= int(main["area"]) * 0.08:
            keep.append(comp)
...
def clear_top_source_slivers(img: Image.Image, row: int) -> Image.Image:
    if row not in (1, 4, 5):
        return img
    cleaned = img.copy()
    pix = cleaned.load()
...
def fit_scaled_crop(crop: Image.Image, source_cell_width: int, row: int, col: int) -> tuple[Image.Image, dict[str, object]]:
    ...
    # The generated source uses generous visual padding; apply a modest
    # body-scale correction so Ascended LAMUH stays close to the approved Sheet 1 size.
    base_scale = (DEST_CELL / source_cell_width) * 1.03
    ...
    vertical_limit = (BASELINE_Y - margin + 1) if grounded else (DEST_CELL - margin * 2)
    fit = min(
        (DEST_CELL - margin * 2) / max(1, scaled_bbox[2] - scaled_bbox[0]),
        vertical_limit / max(1, scaled_bbox[3] - scaled_bbox[1]),
        1.0,
    )
    if fit < 0.999:
    ...
    out = remove_far_detached_fragments(out)
    out = clear_top_source_slivers(out, row)
    out_bbox = content_bbox(out)
    out_body = body_bbox(out)
    counts = count_pixels(out)
    golden_locs_pixels = count_golden_locs_pixels(out, out_body)
    ...
    return out, {
        ...
        "golden_locs_pixels": golden_locs_pixels,
...
def normalize(source: Path, output: Path, preview: Path, report_path: Path, reference_report: Path) -> dict[str, object]:
    ...
    repaired_source_slots = [
        {
            "row": int(slot["row"]),
            "col": int(slot["col"]),
            "from": slot.get("repaired_from"),
        }
        for slot in source_assignment["slots"]
        if slot.get("repaired_from")
    ]
    ...
    for row in range(ROWS):
        ...
        row_summary.append({
            ...
            "golden_locs_pixels": sum(int(c.get("golden_locs_pixels") or 0) for c in row_cells),
    ...
    golden_locs_ok = all(int(c.get("golden_locs_pixels") or 0) > 90 for c in cells)
    ...
    no_important_clipping = all(
        not c.get("clipped") or int(c.get("body_height") or 0) >= 260
        for c in cells
    )
    ...
    report = {
        ...
        "source_repaired_slots": repaired_source_slots,
        ...
        "golden_locs_preserved": golden_locs_ok,
        "occupied_cells": occupied_cells,
        "super_baseline_max_deviation": baseline_max,
        "airborne_center_max_deviation": airborne_center_max,
        "no_important_super_body_clipping": no_important_clipping,
        "body_scale": {
            "sheet1_reference_median_height": sheet1_reference,
            "super_ascended_median_body_height": body_median,
            "ratio": body_ratio,
            "passes": scale_passes,
        },
...
def main() -> int:
    ...
    parser.add_argument(
        "--source",
        default=Path("assets/characters/lamuh/super_ascended/lamuh_super_ascended_golden_locs_source.png"),
        type=Path,
    )
    parser.add_argument(
        "--output",
        default=Path("assets/characters/lamuh/lamuh_sheet_super_ascended_golden_locs_atlas.png"),
        type=Path,
    )
    parser.add_argument(
        "--preview",
        default=Path("assets/characters/lamuh/lamuh_sheet_super_ascended_golden_locs_preview.png"),
        type=Path,
    )
    parser.add_argument(
        "--report",
        default=Path("assets/characters/lamuh/lamuh_sheet_super_ascended_golden_locs_report.json"),
        type=Path,
    )
    parser.add_argument(
        "--sheet1-report",
    ...
    print(json.dumps({
        ...
        "golden_locs_preserved": report["golden_locs_preserved"],
        ...
        "super_baseline_max_deviation": report["super_baseline_max_deviation"],
    ...
    if not report["gold_cyan_accents_preserved"]:
        return 6
    if not report["golden_locs_preserved"]:
        return 12
    ...
    if not report["body_scale"]["passes"]:
        return 8
    if not report["no_important_super_body_clipping"]:
        return 9
    return 0
