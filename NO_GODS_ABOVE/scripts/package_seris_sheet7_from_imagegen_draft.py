#!/usr/bin/env python3
"""Package a generated Seris Sheet 7 draft into the runtime-compatible atlas.

This is an asset-prep helper only. It removes the chroma/grid preview and writes
the exact 3072x2688 transparent Sheet 7 atlas. It does not change runtime code.
"""
...
ROWS = 7
BASELINE_Y = 350
FRAME_COUNTS = [6, 3, 6, 8, 8, 8, 8]
ROW_NAMES = [
    "knockdown_fall_grounded_drop",
    "grounded_downed_idle",
    "get_up_recovery",
    "ko_strong_defeat_finish_state",
    "intro_entrance",
    "victory_win_pose",
    "taunt_flavor_pose",
]
...
def package(input_path: Path, output_path: Path, report_path: Path) -> dict:
    ...
    if len(x_lines) == COLS + 1 and len(y_lines) != ROWS + 1:
        raise ValueError(
            f"Source preview appears to be {COLS}x{len(y_lines) - 1}, not {COLS}x{ROWS}; regenerate Sheet 7."
        )
    ...
    if use_detected_grid:
        warnings.append("Source preview used detected 8x7 grid-line boundaries.")
    else:
...
def main() -> int:
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=Path("assets/sprites/seris_revamp/seris_sheet_7_knockdown_recovery_flavor_revamp_atlas.png"),
    )
    parser.add_argument(
        "--report",
        type=Path,
        default=Path("assets/sprites/seris_revamp/seris_sheet_7_knockdown_recovery_flavor_revamp_pack_report.json"),
    )
