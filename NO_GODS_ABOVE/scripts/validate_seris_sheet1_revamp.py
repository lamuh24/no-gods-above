#!/usr/bin/env python3
"""Validate the Seris Sheet 1 Core Movement revamp atlas before runtime use."""
...
def validate(path: Path, report_path: Path, contact_path: Path) -> dict:
    ...
    if image.mode != "RGBA":
        failures.append("PNG must have RGBA data")
    ...
    opaque_green = int((((rgb[:, :, 0] < 40) & (rgb[:, :, 1] > 220) & (rgb[:, :, 2] < 40) & (alpha > 0))).sum())
    opaque_magenta = int((((rgb[:, :, 0] > 220) & (rgb[:, :, 1] < 40) & (rgb[:, :, 2] > 220) & (alpha > 0))).sum())
    if opaque_green:
        failures.append(f"Opaque chroma green pixels remain: {opaque_green}")
    if opaque_magenta:
    ...
    report = {
        ...
        "contract": {
            "sheet": "Seris Sheet 1 - Core Movement",
