...
$reviewRoot = Join-Path $RepoRoot 'tools\nga-forge\review\lamuh-legacy-v2-standing-medium-v1'
...
$frames = @(
    [ordered]@{ index = 0; role = 'wide_ready_entry'; raw = 'standing-medium-00-wide-ready-v1.png'; rawRootX = 573; rawRootY = 1184; sourceScaleCorrection = 1.04; bodyScaleCorrection = 0.69 },
    [ordered]@{ index = 1; role = 'rear_hand_load'; raw = 'standing-medium-01-rear-hand-load-v1.png'; rawRootX = 600; rawRootY = 1165; sourceScaleCorrection = 0.96; bodyScaleCorrection = 0.70 },
    [ordered]@{ index = 2; role = 'cross_body_hip_coil'; raw = 'standing-medium-02-cross-body-coil-v1.png'; rawRootX = 561; rawRootY = 1091; sourceScaleCorrection = 1.23; bodyScaleCorrection = 0.70 },
    [ordered]@{ index = 3; role = 'rear_cross_acceleration'; raw = 'standing-medium-03-rear-cross-acceleration-v1.png'; rawRootX = 600; rawRootY = 1187; sourceScaleCorrection = 0.92; bodyScaleCorrection = 0.70 },
    [ordered]@{ index = 4; role = 'single_rear_cross_contact'; raw = 'standing-medium-04-rear-cross-contact-body-v1.png'; rawRootX = 573; rawRootY = 1202; sourceScaleCorrection = 1.0; bodyScaleCorrection = 0.70 },
    [ordered]@{ index = 5; role = 'same_arm_shoulder_overshoot'; raw = 'standing-medium-05-shoulder-overshoot-v1.png'; rawRootX = 573; rawRootY = 1198; sourceScaleCorrection = 1.0; bodyScaleCorrection = 0.70 },
    [ordered]@{ index = 6; role = 'rear_arm_recoil'; raw = 'standing-medium-06-rear-arm-recoil-v1.png'; rawRootX = 573; rawRootY = 1200; sourceScaleCorrection = 0.97; bodyScaleCorrection = 0.70 },
    [ordered]@{ index = 7; role = 'connected_guard_recovery'; raw = 'standing-medium-07-guard-recovery-v1.png'; rawRootX = 573; rawRootY = 1199; sourceScaleCorrection = 0.97; bodyScaleCorrection = 0.70 }
)
...
foreach ($frame in $frames) {
    ...
    if (-not (Test-Path -LiteralPath $sourcePath)) { throw "Missing Standing Medium raw candidate: $sourcePath" }
    $outputName = ('standing-medium-{0:d2}.png' -f $frame.index)
...
$contactSource = Join-Path $rawRoot 'standing-medium-04-rear-cross-contact-hit-v1.png'
$contactOutput = Join-Path $normalizedRoot 'standing-medium-04-hit-composite.png'
$contactScale = $sequenceWideScale * $frames[4].sourceScaleCorrection * $frames[4].bodyScaleCorrection
$contactMetrics = [LamuhStandingLightNormalizer]::Normalize($contactSource, $contactOutput, 573, 1202, $targetRoot.x, $targetRoot.y, 2048, 1536, $contactScale)
...
$contactSheetPath = Join-Path $reviewRoot 'standing-medium-numbered-contact-sheet.png'
...
try {
    ...
    $boardGraphics.DrawString('LAMUH LEGACY V2 - STANDING MEDIUM - NUMBERED FRAME SCRUB', $titleFont, [System.Drawing.Brushes]::Goldenrod, 26, 18)
    $boardGraphics.DrawString('Authored facing - fixed root - frame 04 is the only visible contact - candidate only', $noteFont, [System.Drawing.Brushes]::LightGray, 28, 53)
    foreach ($frame in $reportFrames) {
        ...
        if ($frame.index -eq 4) { $boardGraphics.DrawRectangle($goldPen, $panelRectangle) }
        $image = [System.Drawing.Image]::FromFile((Join-Path $normalizedRoot ('standing-medium-{0:d2}.png' -f $frame.index)))
...
$report = [ordered]@{
    schemaVersion = '1.0.0'
    subject = 'lamuh_legacy_v2.standing_medium.single_hit_sequence.candidate.v1'
    ...
    sourceRepairReason = 'V1 one-hit row visibly chains a rear-hand punch into a later high kick. V2 modernization preserves the wide base, deep cross-body coil, rear-hand arc, body momentum, coat and loc lag, and recovery direction while retiring the obsolete second strike to restore one-visible-impact parity.'
    ...
    approvalBoundary = [ordered]@{
        ...
        standingLightMotionApproval = 'APPROVED_V1_MOTION_PRESERVED'
        standingLightTimingApproval = 'APPROVED_V2_RETIMING'
...
Write-Output "Normalized $($reportFrames.Count) Standing Medium candidate frames."
