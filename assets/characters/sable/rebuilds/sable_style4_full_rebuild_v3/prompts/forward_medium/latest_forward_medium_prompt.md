...
# Sprite Clip Prompt: sable / forward_medium
...
Create exactly 10 frames for `forward_medium`.
Show Sable only. Do not include any other character, prop character, weapon, UI card, title, logo, or background scene.

The result must be:

- one horizontal strip sized `4480x448`
- exactly 10 equal cells, each `448x448`
...
full-rebuild-v3 directional normal. Forward medium with more commitment than light and restrained attached VFX, not a teleport or special dash.
